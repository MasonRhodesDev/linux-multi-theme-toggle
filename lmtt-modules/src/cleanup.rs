...
impl CleanupManager {
    ...
    pub async fn run_all(&self) -> Result<()> {
        ...
        println!(
            "This will remove all lmtt-injected config lines from your application configs.\n"
        );
        print!("Continue? [y/N] ");
        io::stdout().flush()?;
        ...
        for module in &self.registry.modules {
            // Skip a module whose config can't be read rather than aborting a
            // half-finished cleanup with no summary.
            let config_files = match module.config_files().await {
                Ok(files) => files,
                Err(e) => {
                    println!("\n{} — skipped: {}", module.name(), e);
                    total_errors += 1;
                    continue;
                }
            };
    ...
    pub async fn run_module(&self, module_name: &str) -> Result<()> {
        ...
        let module = self
            ...
            .ok_or_else(|| {
                lmtt_core::Error::Module(format!("Module not found: {}", module_name))
            })?;
