...
impl CustomModule {
    ...
    pub fn from_file(path: &PathBuf) -> Result<Self> {
        ...
        let definition: CustomModuleDefinition = toml::from_str(&content).map_err(|e| {
...
impl CustomModule {
    ...
    async fn apply_script(&self, scheme: &ColorScheme, script: &ScriptConfig) -> Result<()> {
        ...
        if script.pass_as_env {
            ...
            let temp_file =
                std::env::temp_dir().join(format!("lmtt-{}.json", self.definition.name));
...
pub fn load_custom_modules() -> Result<Vec<CustomModule>> {
    let config_dir = dirs::config_dir().ok_or(lmtt_core::Error::Config("No config dir".into()))?;
