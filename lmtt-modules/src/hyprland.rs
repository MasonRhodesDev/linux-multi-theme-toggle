...
impl ThemeModule for HyprlandModule {
    ...
    async fn apply(&self, scheme: &ColorScheme, _config: &Config) -> Result<()> {
        ...
        // Extract key colors
        let primary = scheme.get("primary").unwrap_or(&"#9fd491".to_string());
        let surface = scheme.get("surface").unwrap_or(&"#12131a".to_string());
        let on_surface = scheme.get("on_surface").unwrap_or(&"#e3e1ec".to_string());
